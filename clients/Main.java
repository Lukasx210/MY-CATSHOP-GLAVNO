...
import clients.imageDisplay.ImageDisplayController;
import clients.imageDisplay.ImageDisplayModel;
import clients.imageDisplay.ImageDisplayView;
...
class Main
{
  ...
  public void startImageDisplayGUI_MVC(MiddleFactory mlf )
    {
  JFrame  window = new JFrame();

  window.setTitle( "Image Display Client MVC");
  window.setDefaultCloseOperation( JFrame.EXIT_ON_CLOSE );
  Dimension pos = PosOnScrn.getPos();

  ImageDisplayModel model      = new ImageDisplayModel(mlf);  // Create ImageDisplayModel
  ImageDisplayView view        = new ImageDisplayView( window, mlf, pos.width, pos.height );
  ImageDisplayController cont  = new ImageDisplayController( view, mlf );
  view.setController( cont );
