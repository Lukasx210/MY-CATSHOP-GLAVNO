...
public class CashierModel extends Observable
{
    ...
    try {
        if (theStock.exists(pn)) {             // Stock Exists?
            ...
            if (pr.getQuantity() >= amount) {    // In stock?
                ...
                theProduct = null;  // Reset theProduct
            }
        } else {                                  // F Stock exists
            theAction =                             // Unknown
   ...
   */
    ...
    if (theBasket != null && theProduct != null) {
        // Adjust stock quantity
        try {
            theStock.addStock(theProduct.getProductNum(), theProduct.getQuantity());
        } catch (StockException e) {
            DEBUG.error("%s\n%s", "CashierModel.removeFromBasket", e.getMessage());
            theAction = e.getMessage();
            setChanged();
            notifyObservers(theAction);
            return;
        ...
        theProduct = null;
        theAction = "Item removed from the basket.";
        theState = State.process;
    } else {
        theAction = "No item to remove from the basket.";
        theState = State.process;
    }
